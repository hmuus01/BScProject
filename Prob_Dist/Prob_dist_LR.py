#In this file the probability distribtuion graph for LR is plotted for 8 random seeds.
#Import Statements
import math
import pandas as pd
import numpy as np
...
#the prob prediction values of  the LR model and the title of the plot
def generate_density_plot(model_name, true_positives, true_negatives):
    if len(true_negatives) < 1 or len(true_positives) < 1:
        return

    def plot_pdf():
        #Use Seaborn distribution plot function to plot the distribution of the true positivevalue(fraud) predictions For LR
        sns.distplot(true_positives, hist=False, kde=True, kde_kws={'linewidth': 1}, label='TP')
        #Use Seaborn distribution plot function to plot the distribution of the true negative value(Legit) predictions For LR
        ...
        plt.savefig('LR' + '_prob_dist.png', bbox_inches='tight')
        #Show the plot
        plt.show()
...
# Print the accuracy and mean scores
print('accuracy mean = ' + str(mean_accuracy))
print('recall mean = ' + str(mean_recall))


#Call the above function with the Logistic Regression fraud and legit probability predictions as a paramter as well as title of the plot
generate_density_plot('LR', tp_probs, tn_probs)
