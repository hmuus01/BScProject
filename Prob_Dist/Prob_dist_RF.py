#In this file the probability distribtuion graph for RF is plotted for 8 random seeds.
#Import Statements
import math
import pandas as pd
import numpy as np
...
#the prob prediction values of  the RF model and the title of the plot
def generate_density_plot(model_name, true_positives, true_negatives):
    if len(true_negatives) < 1 or len(true_positives) < 1:
        return

    def plot_pdf():
        #Use Seaborn distribution plot function to plot the distribution of the true positivevalue(fraud) predictions For RF
        sns.distplot(true_positives, hist=False, kde=True, kde_kws={'linewidth': 1}, label='TP')
        #Use Seaborn distribution plot function to plot the distribution of the true negative value(Legit) predictions For RF
    ...
    plt.ylim([0, 7])
    #call to the plot pdf method if there are still values in the rf array
    plot_pdf()
...
#Call the above function with the Random Forest fraud and legit probability predictions as a paramter as well as title of the plot
generate_density_plot('RF', tp_probs, tn_probs)
